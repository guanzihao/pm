...
begin
		...
		declare	@pickupaddr varchar(100)
		...
		declare @newOrderFromId varchar(36)
		 ...
		 set @taskTypeId='0320ACB2-6E61-4E14-BE1A-DB8F116F9DFE';--业务类型Id
	
	 --声明一个游标
	 Declare wlExcc cursor for
	select  	id 
			,orderNum 
			,norderno 
			,customcode 
			...
			pre_flight,
			c_address
	from [oujian_pmsyn].[dbo].freightexport where is_syn=0;
	
	--打开一个游标
	open wlExcc

	fetch next from wlExcc into  @id 
			,@orderNum 
			,@norderno 
			,@customcode 
			,@Department 
						...
						begin 	
								...
								IF @oldDate=@newDate
									begin 	
										...
										--插入票据
																	insert into oujian_pm.dbo.WL_Flow_EXCC
																								(
																									obj_Id,
																									obj_createDate,
																									obj_updateDate,
																					...
																					select  @counts=count(obj_id) from SHIPPING_IMCC_COMMISSION where order_from_id=@orderfromid;
																						IF @counts>0
																							BEGIN
																								insert into SHIPPING_IMCC_COMMISSION(
																												obj_id,
																												obj_update,
																												obj_create,
																												task_id,
										...
										--执行成功之后修改数据库状态
										update [oujian_pmsyn].[dbo].freightexport SET is_syn=1 where ID=@id;
										end
