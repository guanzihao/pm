...
begin
		...
		declare	@b_telphone
		declare	@b_fax
		declare	@shipment_time
		declare	@delivery_time
		declare	@port_shipment
		declare	@port_destination
		declare	@packing
		declare	@insurance
		declare	@manufactory
		declare	@term_payment
		declare	@inspection
		declare	@arbitration
		declare	@others
		...
		declare @newOrderFromId varchar(36)
		 ...
		 set @taskTypeId='E8EA1196-D803-4040-B309-C65965A4F142';--业务类型Id
	
	 --声明一个游标
	 Declare curCustomimport cursor for
	select  id,ordernum,itemName,contractnum,bussinessNo,customcode,Department,Contractamount,Contractcurrency,contract_date,
							sellers,
							s_address,
							s_telphone,
							s_fax,
	...
	while (@@fetch_status=0)
								...
								IF @oldDate=@newDate
									begin 	
										...
										--插入票据
											...
											select  @counts=count(obj_id) from PM_IMPORT_CONTRACT where order_from_id=@orderFromId;
											IF @counts>0
												BEGIN
													insert into PM_IMPORT_CONTRACT(
																	obj_Id,
																	obj_createDate,
																	obj_updateDate,
																	task_id,
		...
		ELSE
		 ...
		 Fetch Next From curCustomimport into   @id,@ordernum,@itemName,@contractnum,@bussinessNo,@customcode,@Department,@Contractamount,@Contractcurrency,@contract_date,
			@sellers,
			@s_address,
			@s_telphone,
			@s_fax,
