				...
				set @newDate=REPLACE(CONVERT(varchar(100), GETDATE(), 23),'-',''); 
				 ...
				 IF @oldDate=@newDate
					...
					begin 	
						...
						select  @counts=count(obj_id) from CUSTOMS_EXPORTDECLARATION_AGREEMENT where order_from_id=@newOrderFromId;
						IF @counts>0
							BEGIN
								insert into CUSTOMS_EXPORTDECLARATION_AGREEMENT(
												obj_Id,
												obj_createDate,
												obj_updateDate,
												task_id,
			...
			begin
				...
				--插入票据信息
					insert INTO [oujian_pm].dbo.BG_Flow_EMCC(
						...
						select  @counts=count(obj_id) from CUSTOMS_EXPORTDECLARATION_AGREEMENT where order_from_id=@orderFromId;
						IF @counts>0
							BEGIN
								insert into CUSTOMS_EXPORTDECLARATION_AGREEMENT(
												obj_Id,
