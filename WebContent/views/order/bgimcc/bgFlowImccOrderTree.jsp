...
$(function () {
	...
	$('#jstree_<%=name %>').on("changed.jstree", function (e, data) {
	   //alert(data);
		...
		$("#deptUserList").attr("src","${basePath }/pm/order/CustomsDeAgreementimcc/listBgFlowImccOrderDesc/"+data.selected+"/"+data.node.original.hierarchy+"/"+data.node.original.taskTypeId);
	});
});
