...
$(function () {
	...
	$('#jstree_<%=name %>').on("changed.jstree", function (e, data) {
		...
		alert("1--"+data.selected);
		alert("2---"+data.node.original.hierarchy);
		alert("3---"+data.node.original.taskTypeId);
		$("#deptUserList").attr("src","${basePath }/pm/order/ExportCustomsDeAgreementexcc/listBgFlowExccOrderDesc/"+data.selected+"/"+data.node.original.hierarchy+"/"+data.node.original.taskTypeId);
	});
});
