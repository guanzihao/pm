			                            ...
			                            <div id="tab-1" class="tab-pane active">
			                                <div class="panel-body">
			                                    <div class="dataTables_wrapper form-inline">
													<form id="pageForm" action="${basePath }/sup/supcompany/order/listWlFlowExccOrder/${nodeType}/${flowId}/${isCompany}" class="form-horizontal formValidate" method="post">
														<div class="row">
								                            <div class="col-sm-12">
															...
															<table class="">
												                ...
												                <tbody>
												                	<c:forEach items="${pageBean.pageList }" var="item" varStatus="status">
																		<tr id="toolTr_${item[0]}">
																			...
																			<td> 
																			   ...
																			   <c:if test="${not empty item[9]}">
																			       <div style="display:none;">${item[9]}</div>
																			   </c:if>
																			</td>
																			<td>
																			       <c:choose>
																		...
																		<td>
																		          <c:choose>
															                             <c:when test="${item[12] eq '1'}">未提交</c:when>
																		                 <c:when test="${item[12] eq '2'}">未审核</c:when>
																		                 <c:when test="${item[12] eq '3'}">已取消</c:when>
																		                 <c:when test="${item[12] eq '4'}">终止</c:when>
																		                 <c:when test="${item[12] eq '5'}">草稿</c:when>
															                             <c:when test="${item[12] eq '6'}">处理中</c:when>
															                             <c:when test="${item[12] eq '7'}">已完成</c:when>
															                             <c:when test="${item[12] eq '8'}">未执行</c:when>
															                             <c:when test="${item[12] eq '9'}">已提交</c:when>
																		                 <c:when test="${item[12] eq '10'}">服务商已接收</c:when>
															                             <c:when test="${item[12] eq '11'}">服务商已拒绝</c:when>
															                             <c:when test="${item[12] eq '12'}">未分配</c:when>
															                             <c:when test="${item[12] eq '13'}">已分配</c:when>
															                             <c:when test="${item[12] eq '14'}">已终止</c:when>
															                             <c:when test="${item[12] eq '15'}">执行中</c:when>
															                             <c:otherwise>草稿</c:otherwise>
														                           </c:choose>
														                           <input type="text"  style="display:none;" id="state" value="${item[12]}" />
														                            <div style="display:none;margin-bottom:15px;"></div>
																		</td>
																		<c:if test="${isCompany eq '0' }">
																				<td>${item[15]}</td>
																				       ...
																				       <!-- 服务商  -->
																				        <c:choose>
																				               <c:when test="${item[12] eq '9'}">
																					                  ...
																					                  <a class=""target="iframe_PM" href="${basePath }/sup/wuliuexcc/viewCustomsDeAgreement/${item[0] }/${nodeType}/${flowId}/${isCompany}">
																						                   <span> 查看</span>
																					                  </a>
																				               </c:when>
																				              <c:when test="${item[12] eq '10'}">
																				                           <a class=""  onclick="openLayers('添加','${basePath }/sup/orderCccontroller/orderCc/${item[0]}/wlex',true)">
																							                  <span>维护进度</span>
																						                   </a> 
																						                    <a class="" target="iframe_PM" href="${basePath }/sup/wuliuexccc/editExportShippingExCommission/${item[0] }/${nodeType}/${flowId}/${isCompany}">
																						                       <span>修改</span>
																					                        </a>
																						                    <a class=""target="iframe_PM" href="${basePath }/sup/wuliuexcc/viewCustomsDeAgreement/${item[0] }/${nodeType}/${flowId}/${isCompany}">
																						                        <span> 查看</span>
																				       ...
																				       <c:choose>
																		                 <c:when test="${item[12] eq '1'}">
																		                        <a class="" href="javascript:;" onclick="toolTableUpdate('${basePath }/sup/supcompany/order/tijiaoupdatestateorder','${item[0] }','提交订单')">
																					             ...
																					             <a class="" target="iframe_PM" href="${basePath }/sup/wuliuexcc/editExportShippingExCommission/${item[0] }/${nodeType}/${flowId}/${isCompany}">
																						              <span>修改</span>
																					             </a>
																		                 </c:when>
																		                 <c:when test="${item[12] eq '5'}">
																		                        <a class="" href="javascript:;" onclick="toolTableUpdate('${basePath }/sup/supcompany/order/tijiaoupdatestateorder','${item[0] }','提交订单')">
																						              <span>提交</span>
																					             </a>
																					             <a class=""target="iframe_PM" href="${basePath }/sup/wuliuexcc/viewCustomsDeAgreement/${item[0] }/${nodeType}/${flowId}/${isCompany}">
