...
<script src="${basePath }/resource/plugins/layer/extend/layer.ext.js"></script>
<style>
.hidden-mod {
    display: none
}
            ...
            <div class="mo-row util-clearfix">
                ...
                <div class="util-right">
                    <div class="mo-user" data-role="mo-header">
                        <ul class="">
                            <li id="user-info"><a href="${basePath}/sup/company/user/viewUser" target="iframe_PM">我的云贸通</a>ID: <span id="user-code1"
                            ...
                            <li><a
                                href="${basePath }/sup/notice/notice/listNotice" target="iframe_PM">消息
                        ...
                        <div class="row J_mainContent" style="width:100%; background-color:white;">
                             <c:choose>
                                 <c:when test="${not empty url}">
                                      <iframe id="J_iframe" style="max-height: 1500px" onload="Javascript:SetCwinHeight()"  class="J_iframe" name="iframe_PM"  width="100%" scrolling="no" src="${url}" frameborder="0" data-id="${basePath }/organize/user/index" seamless></iframe>
                                 </c:when>
	                            <c:otherwise>
	                                    <iframe id="J_iframe" style="max-height: 1500px" class="J_iframe" name="iframe_PM"  width="100%"  scrolling="no" src="${basePath }/sup/company/homePage/companyHome" frameborder="0" data-id="${basePath }/organize/user/index" seamless></iframe>                        
	                            </c:otherwise>								                                        
                            </c:choose>
                      </div>
		...
		<script type="text/javascript">
			 /*  function reinitIframe() {
				...
				try {
					...
					var height = Math.max(0, dHeight);
					iframe.height = height;
					console.log(height);
				} catch(ex) {}
			}
			var iframeT1 = window.setInterval("reinitIframe()", 200); 
			 */
			 function SetCwinHeight(){
				  var iframeid=document.getElementById("J_iframe"); //iframe id
			...
			function getIndexHome(){
				    $.ajax({
				       type : "post",
			           url : "${basePath }/sup/index/getCompanyUser",
			           dataType : "json",
			           async : false,
			           success:function(data){
			              $("#mo-message-tip").text(data.noticeCount);
			              $("#user-name").text(data.userName);
			              $("#user-code1").text(data.LoginName);
			           }
				    });
				}
