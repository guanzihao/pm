	...
	<div class="row">
		<div class="col-sm-12 tables_search_label" style="margin-left:20px;margin-top:10px;">
				...
				<pm:authCom authCode="info_listAllComLoginLog">
		        	<a class="btn btn-success" href="${basePath }/sup/flow/wmimccs/listCreditCard/${billId}/0">
						<span>信用证开证</span>
					</a>
				</pm:authCom>
				<pm:authCom authCode="sup_listAllComLoginLog">
	            ...
	            <div class="ibox float-e-margins">
	                <div class="ibox-content">
						<div class="dataTables_wrapper form-inline">
							<form id="pageForm" action="${basePath }/sup/flow/wmimccs/listReceipts/${billId}/0" class="form-horizontal formValidate" method="post">
		                        <div class="row">
									<div class="col-sm-12 tables_search_label">
										<pm:authCom authCode="organize_editInterface">
											<a class="btn btn-white btn-sm" href="javascript:;" onclick="openLayerSize('添加','${basePath }/sup/flow/wmimccs/editReceipts/0/${billId }',true,'90%','90%')">
												<span>添加</span>
											</a>
										</pm:authCom>
									...
									<table class="table table-striped table-bordered table-hover dataTable">
										<thead>
											<tr>
												...
												<th class="sorting<c:if test="${searchBean.searchOrderName eq 'a.divDate'}">${searchBean.searchOrderType ? '_desc' : '_asc'}</c:if>" onclick="goSubmitSort('a.divDate', ${!searchBean.searchOrderType})">货款分拆确认时间</th>
												<th class="sorting<c:if test="${searchBean.searchOrderName eq 'a.recAmount'}">${searchBean.searchOrderType ? '_desc' : '_asc'}</c:if>" onclick="goSubmitSort('a.recAmount', ${!searchBean.searchOrderType})">收货款金额</th>
												<th class="sorting<c:if test="${searchBean.searchOrderName eq 'a.recCurrency'}">${searchBean.searchOrderType ? '_desc' : '_asc'}</c:if>" onclick="goSubmitSort('a.recCurrency', ${!searchBean.searchOrderType})">币种</th>
											</tr>
										</thead>
										<tbody>
												...
												<tr id="toolTr_${item[0] }">
													...
													<td>
														<pm:authCom authCode="role_editOrgRole">
															<a class="btn btn-white btn-xs" href="javascript:;" onclick="openLayerSize('修改','${basePath }/sup/flow/wmimccs/editReceipts/${item[0] }/${billId }', true,'90%','90%')">
																<span>修改</span>
															</a>
														</pm:authCom>
														<pm:authCom authCode="organize_listLoginLog">
															<a class="btn btn-xs btn-white" href="javascript:;" onclick="openLayer('查看详情','${basePath }/sup/flow/wmimccs/viewReceipts/${item[0]}',false)">
																<span> 查看详情</span>
															</a>
														</pm:authCom>
													</td>
