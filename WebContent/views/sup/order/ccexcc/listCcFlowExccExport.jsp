	...
	<div class="wrapper wrapper-content animated fadeIn">
		<div class="row">
	    	<div class="col-sm-12 tables_search_label" style="margin-left:20px;margin-top:10px;">
				<a class="btn btn-success" href="${basePath }/sup/flow/ccexccs/listCcFlowExccOrderReceiving/${billId}/0">
					<span>接单登记</span>
				</a>
	        	<a class="btn btn-info" href="javascript:;">
					<span>出库</span>
				</a>
				<a class="btn btn-success" href="${basePath }/sup/flow/ccexccs/listCcFlowExccSigning/${billId}/0">
					<span>签收</span>
				</a>
				<a class="btn btn-success" style="margin-left:600px;" href="${basePath }/sup/orderCccontroller/orderCc/${orderid.orderFromId}/ccex">
					<span>返回</span>
				</a>
	        </div>
	    </div>
	            ...
	            <div class="ibox float-e-margins">
	                <div class="ibox-content">
						<div class="dataTables_wrapper form-inline">
							<form id="pageForm" action="${basePath }/sup/flow/ccexccs/listCcFlowExccExport/${billId}/0" class="form-horizontal formValidate" method="post">
		                        <div class="row">
									<div class="col-sm-12 tables_search_label">
										<pm:authCom authCode="organize_editInterface">
											<a class="btn btn-white btn-sm" href="javascript:;" onclick="openLayerSize('添加','${basePath }/sup/flow/ccexccs/editCcFlowExccNodeExport/0/${billId }',true,'90%','90%')">
												<span>添加</span>
											</a>
										</pm:authCom>
										<pm:authCom authCode="organize_removeInterface">
											<a class="btn btn-white btn-sm" href="javascript:;" onclick="toolTable('${basePath }/sup/flow/ccexccs/removeCcFlowExccNode', 'ids', 'removeToolTableTr', '删除')">
												<span>删除</span>
											</a>
										</pm:authCom>
									</div>
									...
									<table class="table table-striped table-bordered table-hover dataTable">
										<thead>
											<tr>
												...
												<th class="sorting<c:if test="${searchBean.searchOrderName eq 'a.deliveryAddr'}">${searchBean.searchOrderType ? '_desc' : '_asc'}</c:if>" onclick="goSubmitSort('a.deliveryAddr', ${!searchBean.searchOrderType})">送货地址</th>
												<th class="sorting<c:if test="${searchBean.searchOrderName eq 'a.driverSignDate'}">${searchBean.searchOrderType ? '_desc' : '_asc'}</c:if>" onclick="goSubmitSort('a.driverSignDate', ${!searchBean.searchOrderType})">司机签收时间</th>
												<th>操作</th>
											</tr>
										</thead>
										<tbody>
												...
												<tr id="toolTr_${item[0] }">
													...
													<td>
														<pm:authCom authCode="role_editOrgRole">
															<a class="btn btn-white btn-xs" href="javascript:;" onclick="openLayerSize('修改','${basePath }/sup/flow/ccexccs/editCcFlowExccNodeExport/${item[0] }/${billId }', true,'90%','90%')">
																<span>修改</span>
															</a>
														</pm:authCom>
														<pm:authCom authCode="organize_listLoginLog">
															<a class="btn btn-xs btn-white" href="javascript:;" onclick="openLayer('查看详情','${basePath }/sup/flow/ccexccs/viewCcFlowExccNodeExport/${item[0]}/${billId }',false)">
																<span>查看详情</span>
															</a>
														</pm:authCom>
													</td>
