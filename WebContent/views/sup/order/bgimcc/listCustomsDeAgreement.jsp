		...
		<div class="ibox-title">
			...
			<%-- <div style="margin-bottom:50px;">
				<c:forEach items="${pageBean.pageList}" var="item">
					<div>客户公司:<span id="companyinfo">${item[1]}</span></div>
					<div>最新节点:<span id="companyinfo">${item[25]}</span></div>
					<div>最新节点状态:
						<c:if test="${item[26]==1}">
								<span>已完成</span>
						</c:if>
					...
					<div>最新节点更新时间:<span id="companyinfo">${item[27]}</span></div>
				</c:forEach>
			</div> --%>
				...
				<div class="ui2-tab-body ui2-tab-content">
					...
					<div id="myTabContent" class="tab-content" style="border: 1px solid #dedede; border-top: 0px;padding: 5px;">
						<div id="weituohan" class="tab-pane fade"> 
								...
								<div style="text-align:center;width:30%;border:1px solid black;border-left:0px;margin-left:299px;margin-top:-22px;">报关进口</div>
										...
										<c:forEach items="${pageBean.pageList}" var="item">
														...
														<div style="width:50%;float:left;"
																...
																<label class="ui-form-label" data-label="预计出货日期">
																	期望开始时间 :<span class="J-display"> <fmt:formatDate value="${item[29]}" pattern="yyyy-MM-dd"/>  </span>
																</label>
															...
															<div style="width:50%;float:left;"
																...
																data-widget-cid="widget-68">
																<label class="ui-form-label" data-label="预计出货日期">
																	期望完成时间:<span class="J-display"><fmt:formatDate value="${item[30]}" pattern="yyyy-MM-dd"/></span>
																</label>
															...
															<div style="width:50%;float:left;"
																...
																data-widget-cid="widget-68">
																<label class="ui-form-label" data-label="预计出货日期">
																	备注说明:<span class="J-display">${item[31]}</span>
																</label>
															...
															<div style="width:50%;float:left;"
																...
																data-widget-cid="widget-86" style="display: block;">
																<label class="ui-form-label" >
																	客户公司:<span class="J-display" id="usercom">${item[1]}</span></label>
								...
								<div style="float:right;">
									<c:forEach items="${pageBean.pageList}" var="item">
										确认时间：<fmt:formatDate value="${item[32]}" pattern="yyyy-MM-dd"/>
									</c:forEach>
								</div>
						</div>
						<div id="danjuxiangqing" class="tab-pane fade">
						       ...
						       <div class="col-sm-12 tables_search_label">
										<a class="" href="javascript:;" onclick="ajaxTable('${basePath }/sup/order/OrderFrom/saveFlow/${order_code.id}/7994F33B-798E-4495-885B-FB8F2F39B809',  'removeToolTableTr', '添加单据信息')">
											<span>添加进口报关单据</span>
										</a>
									</div>
		                <div class="ibox-content" style="height: 100%;">
	                        ...
	                        <div class="col-sm-9">
	                       		 <iframe id="deptUserList" src="${basePath}/sup/Bgimcc/listBgImcc/${order_code.id}" frameborder="1" width="100%" style="min-height: 580px;"></iframe>
	                        </div>
						</div>
					</div>
						 </div>
						<div class="ui-form ui-form-horizontal ui-form-grouping node-controller-accordion tab-pane fade in active" id="jibenxinxi" data-widget-cid="widget-26" style="display: block;">
										...
										<c:forEach items="${pageBean.pageList}" var="item">
														<div class="edit-block J-content " >
															<div 
																...
																<div class="ui-form-control">
																	<span class="J-display"><fmt:formatDate value="${item[29]}" pattern="yyyy-MM-dd"/></span>
																</div>
															</div>
															<div style="margin-top:4px;"
																...
																<div class="ui-form-control">
																	<span class="J-display"><fmt:formatDate value="${item[30]}" pattern="yyyy-MM-dd"/></span>
																</div>
															</div>
							...
							<article class="step  node-accordion ui-view-mode" id="step03" data-widget-cid="widget-67" style="display: block;">
									...
									<div class="payee-container  view-mode wl2-3">
										<div class="step-content">
											<%--  <pm:execute id="fujian" bean="customsDeAgreementBusinImpl" method="getCustomsDeAgreement">
													<pm:execute-param type="java.lang.String" value="${item[0]}" />
											</pm:execute> --%>
											 <pm:fileList metaObject="${bgic}" delete="false"  metaColums="columsBgIc"/>
										</div>
									</div>
							</article>
						</div>
					...
					<c:if test="${usertype =='sup'}">
						<c:if test="${order_code.orderState =='9'}">
							<a class="btn btn-white btn-sm myBtn" href="${basePath }/sup/supcompany/order/supupdateorderoks/${order_code.id}/${nodeType}/${flowId}/${isCompany}/bgim">
								<span> 接收</span>
							</a>
							<a class="btn btn-white btn-sm myBtn" href="${basePath }/sup/supcompany/order/supupdateorders/${order_code.id}/${nodeType}/${flowId}/${isCompany}/bgim">
								<span> 拒绝</span>
							</a>
						</c:if>
					</c:if>
