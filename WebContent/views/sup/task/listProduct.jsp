...
<style>
<!--
	.tables_search_label input[type="text"]{
	                ...
	                <div class="ibox-content">
						<div class="dataTables_wrapper form-inline">
							<form id="pageForm" class="form-horizontal formValidate" action="${basePath }/sup/task/listProduct/${taskTypeId}/${flag}" method="post">
								<input type="hidden" id="updatewmIcId" value="${updatewm }">
								<div class="row">
		                            <div class="col-sm-12">
		                                ...
		                                <label class="tables_search_label">
			                             ...
			                             <label class="tables_search_label">
		                                	产品目录：<input type="hidden" name="productTypeId" id="productTypeId" value="${productTypeId}" ><input type="text" id="productTypeName" name="productTypeName" class="form-control my_input" value="${productTypeName }"  onclick="selectProductType()" >
		                                  </label>	
		                                <label class="tables_search_label">
										...
										<tbody>
											<c:forEach items="${pageBean.pageList }" var="item" varStatus="status">
												<tr id="toolTr_${item[0]}" ondblclick="hide('${item[0]}');">
													<td>
														<img alt="点击带回数据" onclick="hide('${item[0]}');" onclick="hide('${item[0]}');" style="width: 30px;height: 30px;" src="${basePath }/resource/images/daihui.png">
													</td>
													<td>
...
function hide(ids){
	...
	var updateId=$("#updatewmIcId").val();
	if (updateId=='updatewmIc') {
		parent.returnValueSpUpdateWmIc(toValue);
	}else if (updateId=='updatewmEc') {
		parent.returnValueSpUpdateWmEc(toValue);
	} else{
		parent.returnValueSpWmIc(toValue);
	}
