...
<link href="${basePath }/resource/css/pro.css" rel="stylesheet" />
...
<script src="${basePath }/resource/js/echarts.min.js"></script>
	<body class="gray-bg" style="height:900px">	
		...
		<div class="white_place" id="white_place" >
         <div class="bg_zzt" style="margin-top:15px;height: 210px;">
		   ...
		   <div class="bg_zzt" style="    margin-left: 15px;margin-top:15px;height: 210px;margin-bottom: 20px">
			...
			<div style="font-size: 16px ;z-index: 1"> 您有<font><a href="${basePath }/order/listStatusOrder?searchName1=88" style="color: red;">${orderNumber}</a></font>个订单 </div>
			...
			<div style="text-align: center;">
			    <div id="main1" style="width: 440px;height:350px;float:left;"></div>
				<div id="main2" style="width: 440px;height:350px;float:left;"></div>
				<div id="main3" style="width: 440px;height:350px;float:left;"></div>
				<div id="main4" style="width: 440px;height:350px;float:left;"></div>
			</div>
		</div>
				<div class="bg_zzt" style="margin-top:15px;margin-left: 15px;height:400px;">
			<div class="bg_txt" > 客户发布订单数量及占比</div>
			<div style="text-align: center;">
			     <div id="echartsPie" style="width: 456px;height:300px;margin-left:18px;"></div>
			</div>
		</div>
		</div>
...
<script type="text/javascript">
	...
	function toDetailPage(obj){
	   var id=$(obj).next().val();
	   window.location.href="${basePath}/noticeinformed/noticeinformed/viewSNoticeInformed/"+id;
	}
	function toDetailNoticePage(obj){
	    var id=$(obj).next().val();
	    window.location.href="${basePath}/notice/notice/viewNotice/"+id;
	}
	function showProp(obj) {
		$.ajax({
			type : "post",
			url : "${basePath }/organize/customIndex/SearchBillProp",
			dataType : "json",
			async : false,
	...
	function showBar(echarts,checkData) {
		$.ajax({
			type : "post",
			url : "${basePath }/organize/customIndex/SearchOrderNum",
			dataType : "json",
			async : false,
	...
	$(".submitClass").on("click",function(){
		window.location.href='${basePath }/order/listStatusOrder?searchName1=9';
	});
	$(".distributionClass").on("click",function(){
		window.location.href='${basePath }/order/listStatusOrder?searchName1=13';
	});
	$(".accomplishClass").on("click",function(){
		window.location.href='${basePath }/order/listStatusOrder?searchName1=7';
	});
</script>
