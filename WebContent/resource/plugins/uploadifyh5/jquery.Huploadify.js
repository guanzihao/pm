...
$.fn.Huploadify = function(opts){
	...
	this.each(function(){
		...
		var inputStr = '<input id="select_btn_'+instanceNumber+'" class="selectbtn" style="display:none;" type="file" name="fileArray[]"';
				  ...
				  for(var i=0,len=files.length;i<len;i++){
				  	var thisFile = files[i];
				  		if(parseInt(formatFileSize(thisFile.size,true))>option.fileSizeLimit){
				  			parent.layer.msg('文件'+thisFile.name+'大小超出限制！',{icon: 2});
				  			alert();
				  			continue;
				  		}
						if($.inArray(thisFile.name.split('.').pop(),typeArray)>=0){
							...
							parent.layer.msg('文件'+thisFile.name+'类型不允许！',{icon: 2});
						}  	
					}	
				}
			  return arr;  	
